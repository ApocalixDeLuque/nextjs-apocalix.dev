...
function CopyClipboard() {
    ...
    const copyToClipboard = async () => {
        try {
            ...
            setCopySuccess(`me@apocalix.dev - ${getText('copied!','copiado!')}`)
        } catch (err) {
            setCopySuccess(`me@apocalix.dev - ${getText('there was a problem','hubo un problema')} :(`)
        }
    ...
    return (    
        <CustomTooltip followCursor title={<p>{getText("copy to clipboard","copiar al portapapeles")}</p>}>
            <p className='text-lg md:text-[28px] xl:text-[32px] 2xl:text-[40px] 3xl:text-5xl hover:text-red transition-all duration-[250ms] hover:cursor-pointer select-none text-start' onClick={copyToClipboard}>
...
const Contact = () => {
    ...
    const [formSeverity, setFormSeverity] = useState<AlertColor | undefined>(undefined) 
    ...
    const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setFormMessage(getText("email sent correctly!","¡correo enviado correctamente!"))
            setFormSeverity("success")
        } catch (error) {
            setFormMessage(getText("there was a problem, try again later","hubo un problema, intenta más tarde"))
                        ...
                        <div className='flex items-center gap-1 xxs:gap-4'>
                            <CustomTooltip followCursor title={<p>{getText("let's connect on linkedin!","¡conectémos en linkedin!")}</p>}>
                                <Link scroll={false} href='https://www.linkedin.com/in/apocalix/'>
                                    <Image className='h-6 xxs:h-8 lg:h-10 w-auto hover:scale-110 transition-all' src={linkedin} alt='linkedin'/>
                                </Link>
                            </CustomTooltip>
                            <CustomTooltip followCursor title={<p>{getText("send me an email!","¡mándame un correo!")}</p>}>
                                <Link scroll={false} href='mailto:contact@apocalix.dev'>
                                    <Image className='h-6 xxs:h-9 lg:h-12 w-auto hover:scale-110 transition-all' src={mail} alt='mail'/>
                                </Link>
                            </CustomTooltip>
                        </div>
                    </div>
                    
                    <form onSubmit={onSubmit} className='w-full flex flex-col gap-3 select-none'>
                        ...
                        <div className='flex justify-between'>
                            <div className='flex items-start'>
                                {formMessage && <Alert severity={formSeverity}>{formMessage}</Alert>}
                            </div>
                            <button type="submit" className='flex self-end py-3 px-6 bg-red rounded-[16px] gap-2 hover:bg-green transition-all duration-[250ms] group hover:cursor-pointer' disabled={isSubmitting}>
                                <p className='text-light group-hover:text-light transition-all duration-[250ms]'>
                                    {isSubmitting ? 'Sending...' : hasSubmitted ? 'Sent' : 'Send'}
                                </p>
                            </button>
                        </div>
                    </form>
