        ...
        <CustomTooltip size={fontSize} followCursor placement={placement} title={<p>{tooltipText}</p>}>
            <div
                className={`w-fit flex items-center ${
                    divClassName.includes('flex-row-reverse') ? 'flex-row-reverse' : 'flex-row'
                    ...
                    <div className="w-full flex gap-8 items-center justify-between flex-wrap">
                        <div className="flex flex-col justify-items-start gap-4 md:gap-8 xl:gap-12">
                            <TextList
                                ...
                                text={getText('computer science student', 'estudio sistemas computacionales')}
                            />
                            <TextList
                                tooltipText={getText('lived 8 years in usa!', '¡vivi 8 años en estados unidos!')}
                                text={getText('english-spanish bilingual', 'bilingüe ingles-español')}
                            />
                            <TextList
                                tooltipText={getText('minecraft fan:)', 'fan de minecraft:)')}
                                text={getText('first developed due to videogame passion', 'empecé a desarrollar por pasión a los videojuegos')}
                            />
                            <TextList
                                tooltipText={getText('self taught webstack', 'autodidacta en webstack')}
                                text={getText('aim to become a fullstack dev', 'apunto a ser desarrollador fullstack')}
                            />
                            <TextList
                                tooltipText={getText('click to see my charts!', '¡click para ver mis charts!')}
                        ...
                        <div className="aboutme__container-list_secret">
                            <TextList
                                ...
                                placement="bottom"
                                tooltipText={getText('unimportant facts', 'datos no importantes')}
                                text="🤫"
                            ></TextList>
                        </div>
                    </div>
