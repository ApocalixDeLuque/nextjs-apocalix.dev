        ...
        >
            <div
                className={`w-fit flex items-center ${
                    divClassName.includes('flex-row-reverse')
                        ? 'flex-row-reverse'
                        : 'flex-row'
                } gap-3 md:gap-6${divClassName}`}
                onClick={() => {
                    if (pageURL) window.open(pageURL, '_blank');
                }}
            >
                <Image
                    className="flex w-4 md:w-8 aspect-square justify-center items-center"
                    src={dbarrow}
                    alt="arrow"
                />
                <p className="text-lg md:text-[28px] xl:text-[32px] 2xl:text-[40px] 3xl:text-5xl">
                    {text}
                </p>
            </div>
        </CustomTooltip>
    ...
    return (
        <div className="lg:w-full lg:max-w-[200px] flex flex-col justify-center items-center gap-4 xl:gap-6 2xl:gap-8 hover:cursor-pointer hover:scale-110 transition-transform duration-300">
            <Image
                ...
                alt="icon"
            />
            <p className="max-w-[100px] sm:max-w-[200px] w-full text-center text-lg md:text-xl lg:text-2xl xl:text-2xl 2xl:text-3xl">
                {text}
            </p>
        </div>
    );
...
const AboutMe = () => {
    ...
    return (
        <section
            id={getText('about-me', 'sobre-mi')}
            className="w-full flex items-center justify-center pt-24 md:pt-32 px-8 sm:p-16 xl:pt-80 xl:px-32"
        >
            <div className="w-full h-fit flex flex-col gap-16 xl:gap-20 2xl:gap-32 justify-center max-w-[1800px]">
                <div className="w-full flex flex-col items-center justify-center gap-8 sm:gap-16 xl:gap-32">
                    ...
                    <div className="w-full flex gap-8 items-center justify-between flex-wrap">
                        <div className="flex flex-col justify-items-start gap-4 md:gap-8 xl:gap-12">
                            <TextList
                                tooltipText={'@ universidad de guanajuato'}
                                text={getText(
                            ...
                            <TextList
                                tooltipText={getText(
                                    'lived 8 years in usa!',
                                    '¡vivi 8 años en estados unidos!'
                                )}
                                text={getText(
                            ...
                            <TextList
                                ...
                                text={getText(
                                    'first developed due to videogame passion',
                                    'empecé a desarrollar por pasión a los videojuegos'
                                )}
                            />
                            <TextList
                                tooltipText={getText(
                                    'self taught frontend & design',
                                    'autodidacta en frontend y diseño'
                                )}
                                text={getText(
                                    'aim to become a fullstack dev',
                                    'apunto a ser desarrollador fullstack'
                                )}
                            />
                            <TextList
                                tooltipText={getText(
                                    'click to see my charts!',
                                    '¡click para ver mis charts!'
                                )}
                                text={getText('music lover', 'amo la música')}
                                ...
                                pageURL={
                                    'https://www.last.fm/es/user/ApocalixDeLuque'
                                }
                            />
                        </div>
                        <div className="aboutme__container-list_secret">
                            <TextList
                                divClassName=" flex-row-reverse justify-items-end opacity-20 hover:opacity-100 hover:cursor-pointer transition-all duration-[250ms]"
                                placement="bottom"
                                tooltipText={getText(
                                    'unimportant facts',
                ...
                <div className="flex flex-col gap-20 md:gap-28 xl:gap-36 xl:pt-80">
                    <div className="w-full text-center flex flex-col justify-center items-center gap-12 sm:gap-16 md:gap-24 xl:gap-32">
                        <h2 className="text-3xl 2xl:text-4xl 3xl:text-5xl">
                            {getText('languages i know', 'lenguajes que sé')}:
                        </h2>
                    ...
                    <div className="w-full flex flex-col justify-center items-center gap-12 sm:gap-16 md:gap-24 xl:gap-32">
                        <h2 className="text-3xl 2xl:text-4xl 3xl:text-5xl">
                            {getText('daily tools', 'herramientas')}:
                        </h2>
                        <div className="w-full flex flex-wrap justify-evenly items-center gap-4 md:gap-6 xl:px-24">
