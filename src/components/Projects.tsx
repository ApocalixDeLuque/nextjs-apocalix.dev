...
const ProjectCard = ({title, preview, pageURL, githubURL}: ProjectCardProps) => {
    ...
    return (
        <div className='w-full flex flex-col items-center sm:items-start gap-4 border-2 border-lightgray rounded-[32px] p-4 sm:p-8 xl:pt-9 2xl:p-10 hover:scale-[1.02] transition-all duration-[250ms]'>
            <h2 className='text-3xl md:text-4xl 2xl:text-5xl sm:pb-2 xl:pb-4'>{title}</h2>
            ...
            <div className='w-full flex flex-col lg:flex-row items-center justify-center gap-2'>
                <div className='w-full flex items-center justify-center py-2 sm:py-3 md:py-4 px-4 2xl:py-8 2xl:px-8 gap-2 lg:gap-4 rounded-[32px]  bg-lightgray  hover:bg-red hover:text-light hover:cursor-pointer group transition-all duration-[250ms]' onClick={() => window.open(githubURL, "_blank")}>
                    <Image className='w-5 sm:w-6 lg:w-8 2xl:w-10 3xl:w-12 aspect-square group-hover:invert transition-all duration-[250ms]' src={web} alt='webpage'></Image>
                    <p className='text-2xl xl:text-3xl 2xl:text-4xl'>{getText("see live","en vivo")}</p>
                </div>
                <div className='w-full flex items-center justify-center py-2 sm:py-3 md:py-4 px-4 2xl:py-8 2xl:px-8 gap-2 lg:gap-4 rounded-[32px] bg-lightgray hover:bg-red hover:text-light hover:cursor-pointer group transition-all duration-[250ms]' onClick={() => window.open(githubURL, "_blank")}>
                    <Image className='w-5 sm:w-6 lg:w-8 2xl:w-10 3xl:w-12 aspect-square group-hover:invert transition-all duration-[250ms]' src={github} alt='github'></Image>
